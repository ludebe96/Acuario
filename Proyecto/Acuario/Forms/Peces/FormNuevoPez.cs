...
namespace Acuario.Forms
{
    public partial class FormNuevoPez : MetroFramework.Forms.MetroForm
    {
        ...
        private int CrearPrecio()
        {
            ...
            if (!textboxPrecioMayorista.Text.Trim().Equals(String.Empty))
                precioMayorista = ManagerFormats.Instance.MoneyToDecimal(textboxPrecioMayorista.Text);

            if (!textboxPrecioOferta.Text.Trim().Equals(String.Empty))
                precioOferta = ManagerFormats.Instance.MoneyToDecimal(textboxPrecioOferta.Text);

            if (!textboxPrecioDistribuidor.Text.Trim().Equals(String.Empty))
        ...
        private void CrearPez()
        {
            ...
            if (!textboxStock.Text.Trim().Equals(String.Empty))
        ...
        private void comboboxVariedades_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (comboboxEspecies.SelectedIndex != 0)
            {
                textboxNombre.Text = comboboxEspecies.SelectedItem.ToString();

                if (comboboxVariedades.SelectedIndex != 0)
                    textboxNombre.Text += " " + comboboxVariedades.SelectedItem.ToString();
            }
        }
